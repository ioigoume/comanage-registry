...
class AppModel extends Model {
  ...
  public function typeInUse($attribute, $attributeType, $coId) {
    ...
    if(array_key_exists('co_person_id', $this->getColumnTypes())) {                // Is this model attached to CO Person
      $args['joins'][0]['table'] = 'co_people';
      $args['joins'][0]['alias'] = 'CoPerson';
      $args['joins'][0]['type'] = 'INNER';
      $args['joins'][0]['conditions'][0] = 'CoPerson.id=' . $this->alias . '.co_person_id';
      $args['conditions']['CoPerson.co_id'] = $coId;
    } elseif(array_key_exists('co_person_role_id', $this->getColumnTypes())) {     // Is this model attached to CO Person Role
      ...
      $args['conditions']['CoPerson.co_id'] = $coId;
    } elseif ($this->alias === 'CoDepartment'){              // This attribute is attached to a Department
  ...
  /**
   * @param $a    Field with passed values
   * @param $d    Type of value, e.g INT if we need to apply FILTER_VALIDATE_INT filter_Var
   * @return bool|string
   */
  public function validateMultipleSelect($a, $d = 'INT') {
    if(empty($a)) {
      return true;
    }
    ...
    if(empty(Hash::filter($a[$field]))) {
      return true;
    }

    $filter = constant("FILTER_VALIDATE_" . $d);
    $a_flat = Hash::flatten($a);

    foreach ($a_flat as $k => $v) {
      if (!filter_var($v, $filter)) {
        // Mismatch, implying bad input
        return _txt('er.input.invalid');
      }
    }

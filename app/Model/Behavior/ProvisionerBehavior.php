...
class ProvisionerBehavior extends ModelBehavior {
  ...
  private function invokePlugin($coProvisioningTarget, $provisioningData, $action) {
    ...
    if(!empty($coProvisioningTarget['plugin'])) {
      ...
      $this->log(get_class($this)."::{$fn}::pluginTarget => ".print_r($pluginTarget, true), LOG_DEBUG);
      $this->log(get_class($this)."::{$fn}::coProvisioningTarget => ".print_r($coProvisioningTarget, true), LOG_DEBUG);
      $this->log(get_class($this)."::{$fn}::pluginModelName => ".$pluginModelName, LOG_DEBUG);
      if(!empty($pluginTarget)) {
        try {
          //$this->log(get_class($this)."::{$fn}::pluginmodel => ".print_r($pluginModel, true), LOG_DEBUG);
          $pluginModel->provision($pluginTarget, $pAction, $provisioningData);

          $this->log(get_class($this)."::{$fn}::reached this point", LOG_DEBUG);

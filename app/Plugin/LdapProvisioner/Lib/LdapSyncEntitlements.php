...
class LdapSyncEntitlements {
  ...
  private function groupEntitlementAssemble($memberships_groups){
    ...
    foreach($memberships_groups as $group) {
      ...
      $voGroupPrefix = LdapSyncEntitlements::get_vo_group_prefix($this->config['vo_group_prefix'], $this->coId);
  ...
  public function getEntitlements($coPersonId) {
    ...
    $coGroupMemberships = LdapSyncEntitlements::getMemberships($this->coId, $coPersonId);
    ...
    $this->nested_cous_paths = LdapSyncEntitlements::getCouTreeStructure($cou_memberships);
